 ...
 */
export const authenticateUser = async (email: string, apiKey: string, baseUrl: string): Promise<AxiosResponse<ApiResponse<AuthResponse>>> => {
    try {
 ...
 */
export const logoutUser = async (baseUrl: string, apiKey: string): Promise<AxiosResponse | null> => {
    try {
        const response = await axios({
