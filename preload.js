...
contextBridge.exposeInMainWorld('electronAPI', {
  ...
  showErrorBox: (title, message) => 
    ipcRenderer.invoke('showErrorBox', { title, message }),
  
  // Show info dialog box
  showInfoBox: (message, type = 'info', buttons = ['OK']) => 
    ipcRenderer.invoke('showInfoBox', { message, type, buttons }),
  
  // Show close confirmation dialog when uploads are in progress
  showCloseConfirmBox: () => 
    ipcRenderer.invoke('showCloseConfirmBox'),
  
  // Show file name change confirmation dialog
  showFileNameChangeConformation: (message) => 
    ipcRenderer.invoke('showFileNameChangeConformation', message),

  // ==================== Upload Tracking ====================
  
  // Update upload status tracking (used to warn on close)
  closeActionCheck: (data) => 
  ...
  keepLoginDetails: (apiKey, baseUrl) => 
  ...
  isEncryptionAvailable: () => 
    ipcRenderer.invoke('isEncryptionAvailable'),
  
  // Store a value securely (encrypted)
  secureStore: (key, value) => 
    ipcRenderer.invoke('secureStore', { key, value }),
  
  // Retrieve a securely stored value
  secureRetrieve: (key) => 
    ipcRenderer.invoke('secureRetrieve', { key }),
  
  // Delete a securely stored value
  secureDelete: (key) => 
    ipcRenderer.invoke('secureDelete', { key }),
